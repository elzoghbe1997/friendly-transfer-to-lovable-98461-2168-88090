...
import { AppContextType, FertilizationProgram, CropCycle, TransactionType } from '../types';
...
const ProgramFormModal: React.FC<{
    ...
    onSave: (p: Omit<FertilizationProgram, 'id'> | FertilizationProgram) => void; 
    onClose: () => void; 
    cropCycles: CropCycle[];
    selectedCycleId?: string;
}> = ({ program, onSave, onClose, cropCycles, selectedCycleId }) => {
    ...
    const [cropCycleId, setCropCycleId] = React.useState(program?.cropCycleId || selectedCycleId || '');
    
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!cropCycleId) { 
            addToast('يرجى اختيار الدورة الزراعية.', 'error'); 
            return; 
        }
    ...
    return (
        <div className="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-lg shadow-xl w-full max-w-lg" onClick={e=>e.stopPropagation()}>
                <h2 className="text-2xl font-bold mb-4">{program ? 'تعديل البرنامج' : 'إضافة برنامج جديد'}</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="cropCycle" className="block text-sm font-medium">الدورة الزراعية</label>
                        <select 
                            id="cropCycle" 
                            value={cropCycleId} 
                            ...
                            disabled={!!program}
                        >
                            <option value="">اختر الدورة الزراعية</option>
                    ...
                    <div className="flex justify-end space-x-2 space-x-reverse pt-4">
                        ...
                        <button type="submit" className="px-4 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700">حفظ</button>
                    </div>
                </form>
            </div>
        </div>
...
const ProgramReportModal: React.FC<{program: FertilizationProgram; onClose: () => void;}> = ({ program, onClose }) => {
    ...
    if (!reportData) return (
        <div className="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
            <div className="bg-white p-4 rounded-lg">خطأ في تحميل البيانات.</div>
        </div>
    );
        ...
        <div className="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col" onClick={e=>e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4 pb-4 border-b border-slate-200 dark:border-slate-700">
                    <div>
                        <h2 className="text-2xl font-bold text-slate-800 dark:text-white">تقرير: {program.name}</h2>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">الدورة الزراعية: {cycle.name}</p>
                    </div>
                ...
                <div className="flex-grow overflow-y-auto space-y-6 modal-scroll-contain">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <ReportStatCard 
                            title="إجمالي مصروفات البرنامج" 
                            value={formatCurrency(totalExpenses)} 
                            icon={<ExpenseIcon className="w-7 h-7 text-rose-500" />} 
                        />
                        <ReportStatCard 
                            title="إيرادات البرنامج" 
                            value={formatCurrency(totalRevenue)} 
                            icon={<RevenueIcon className="w-7 h-7 text-emerald-500" />} 
                        />
                        <ReportStatCard 
                            title="صافي ربح المالك" 
                            value={formatCurrency(ownerProfit)} 
                            icon={<ProfitIcon className={`w-7 h-7 ${ownerProfit >= 0 ? 'text-sky-500' : 'text-orange-500'}`} />} 
                        />
                    </div>
                    {settings.isFarmerSystemEnabled && cycle.farmerId && (
                        <ReportStatCard 
                            title={`حصة المزارع (${cycle.farmerSharePercentage}%)`} 
                            value={formatCurrency(farmerShare)} 
                            icon={<FarmerIcon className="w-7 h-7 text-indigo-500" />} 
                        />
                    )}
                                ...
                                <table className="w-full text-sm">
                                    <tbody>
                                        {expenses.length > 0 ? expenses.map(t => (
                                            <tr key={t.id} className="border-b dark:border-slate-700 last:border-b-0">
                                                <td className="p-2">{t.description}</td>
                                                <td className="p-2 text-left font-semibold">{formatCurrency(t.amount)}</td>
                                            </tr>
                                        )) : (
                                            <tr>
                                                <td colSpan={2} className="p-4 text-center text-slate-500">لا توجد مصروفات.</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
                                ...
                                <table className="w-full text-sm">
                                    <tbody>
                                        {revenues.length > 0 ? revenues.map(t => (
                                            ...
                                            <tr>
                                                <td colSpan={2} className="p-4 text-center text-slate-500">لا توجد إيرادات.</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
...
const FertilizationProgramsPage: React.FC = () => {
    const { fertilizationPrograms, transactions, cropCycles, addFertilizationProgram, updateFertilizationProgram, deleteFertilizationProgram, settings } = React.useContext(AppContext) as AppContextType;
    ...
    const [filterCycleId, setFilterCycleId] = React.useState<string>('all');
    ...
    if (!settings.isAgriculturalProgramsSystemEnabled) {
        return (
            <div className="text-center p-8 bg-white dark:bg-slate-800 rounded-lg shadow-md">
                <ProgramIcon className="w-16 h-16 text-slate-400 mx-auto mb-4"/>
                <p className="text-slate-600 dark:text-slate-400">نظام البرامج الزراعية غير مفعّل حالياً.</p>
            </div>
        );
    }

    const filteredPrograms = React.useMemo(() => {
        const programs = filterCycleId === 'all' 
            ? fertilizationPrograms 
            : fertilizationPrograms.filter(p => p.cropCycleId === filterCycleId);
        return programs.sort((a,b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime());
    }, [fertilizationPrograms, filterCycleId]);
    ...
    return (
        <div className="space-y-6">
            <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <h1 className="text-3xl font-bold">البرامج الزراعية</h1>
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                    <select 
                        value={filterCycleId} 
                        onChange={e => setFilterCycleId(e.target.value)}
                        className="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-md"
                    >
                        <option value="all">جميع الدورات الزراعية</option>
                        {cropCycles.map(cycle => (
                            <option key={cycle.id} value={cycle.id}>{cycle.name}</option>
                        ))}
                    </select>
                    <button 
                        onClick={() => { setSelectedProgram(undefined); setModal('ADD'); }} 
                        className="flex items-center justify-center px-4 py-2 bg-emerald-600 text-white rounded-md shadow-sm hover:bg-emerald-700"
                    >
            ...
            {filteredPrograms.length > 0 ? (
                            ...
                            <div key={program.id} className="bg-white dark:bg-slate-800 p-5 rounded-lg shadow-md hover:shadow-xl transition-shadow flex flex-col justify-between">
                                <div>
                                    ...
                                    <div className="mt-4 border-t border-slate-200 dark:border-slate-700 pt-3 grid grid-cols-3 gap-2 text-center">
                                        ...
                                        <div>
                                            <p className="text-xs text-slate-500">ربح المالك</p>
                                            <p className={`text-sm font-semibold ${ownerProfit >= 0 ? 'text-sky-600' : 'text-orange-500'}`}>
                                ...
                                <div className="flex justify-between items-center mt-4 border-t border-slate-200 dark:border-slate-700 pt-3">
                                    <button 
                                        onClick={() => { setSelectedProgram(program); setModal('VIEW_REPORT'); }} 
                                        className="flex items-center px-3 py-1.5 text-xs bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-md hover:bg-slate-200 dark:hover:bg-slate-600"
                                    >
                                    ...
                                    <div>
                                        <button 
                                            onClick={() => { setSelectedProgram(program); setModal('EDIT'); }} 
                                            className="p-2 text-slate-400 hover:text-blue-500 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700"
                                        >
                ...
                <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-lg border-2 border-dashed border-slate-200 dark:border-slate-700">
                    ...
                    <p className="text-lg font-semibold text-slate-600 dark:text-slate-300">
                        {filterCycleId === 'all' ? 'لا توجد برامج مسجلة' : 'لا توجد برامج لهذه الدورة الزراعية'}
                    </p>
            ...
            {(modal === 'ADD' || modal === 'EDIT') && (
                <ProgramFormModal 
                    ...
                    selectedCycleId={filterCycleId !== 'all' ? filterCycleId : undefined}
                />
            )}
            ...
            <ConfirmationModal 
                ...
                message="هل أنت متأكد من حذف هذا البرنامج؟ لا يمكن حذف برنامج مرتبط بمعاملات."
            />
        </div>
    );
};
